...
exports.jobs = {
	create: function(recipients, address, destination, scheduledTime, pickupAddress) {
		...
		if (destination) query.body.destination = destination;
		if (scheduledTime) query.body.scheduledTime = scheduledTime;
		if (pickupAddress) query.body.pickupAddress = pickupAddress;
		...
		console.log('CREATE JOB OPTIONS:',query)
		request.post(
			options,
