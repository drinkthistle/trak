...
function setOptions(endpoint) {
	var options = {
		...
		headers: {
			'trak-api-key': process.env.TRAK_KEY
		},
		json: true
	};
...
exports.jobs = {
	create: function(recipients, address, destination, scheduledTime, pickupAddress) {
		...
		if (destination) trakOptions.body.destination = destination;
		if (scheduledTime) trakOptions.body.scheduledTime = scheduledTime;
		if (pickupAddress) trakOptions.body.pickupAddress = pickupAddress;
		...
		request.post(
			options,
			function(error, response, trakOptions.body) {
			...
			function(error, response, body) {
				if (!error && response.statusCode == 200) {
					...
					_.each(body, function(driver) {
						if(driver.jobs){
								driver.jobs.parsed=JSON.stringify(driver.jobs)
